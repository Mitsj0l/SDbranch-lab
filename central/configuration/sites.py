...
def sites_assigndevice(site_id,serial_number):
    try:
        print("Associating device: {} to location_ID: {} .".format(serial_number, site_id))
...
def sites_bulkupload():
    ...
    for (idx, row) in df.iterrows():
        ...
        try:
            print("Creating site location: {} - {} - {}.".format(site_name,site_address,site_country))
            response = requests.post(
                url= headers.central_instance + "/central/v2/sites",
                headers= headers.set_header_backend(),
                data=json.dumps({
...
def sites_delete():
    ...
    print(sites)
    site_location_ids = sites['site_id'][sites['site_name'].str.contains("SDB-")].tolist() # Filtering "SDB-" Keyword in the Site names and then receive the site_id's of those entries.
    ...
    for site in site_location_ids:
        try:
            ...
            response = requests.delete(
                url= headers.central_instance + "/central/v2/sites/{}".format(site),
                headers= headers.set_header_backend(),
            )
            print('Response HTTP Status Code: {status_code}'.format(
                status_code=response.status_code))
