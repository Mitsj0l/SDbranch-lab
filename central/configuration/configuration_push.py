...
def config_device(filename, device_name, serial_number, mac_address):
    try:
        print("Push the per-device configuration to device {} - {} - {} .".format(device_name, serial_number, mac_address))
...
def groupconfig_create(central_group_name):
    for groupname in central_group_name:
        ...
        # print(bgw_csv2)
        try:
            print("Pushing group configuration of filename {} to Central group {}.".format(filename_directory,groupname))
            response = requests.post(
                url= headers.central_instance + "/caasapi/v1/exec/cmd",
                params={
                    "cid": headers.app_customerid,
