...
central_group_name = [central_group_bgw, central_group_vpnc1, central_group_vpnc2]
...
def Assignment_VGW(array_central_group_bgw, array_central_group_vpnc1, array_central_group_vpnc2):
    ...
    central_group_id = [array_central_group_vpnc1[0][1] ,array_central_group_vpnc2[0][1],array_central_group_bgw[0][1] ,array_central_group_bgw[0][1]] #Retrieve 2nd Array object the group_id
    central_group_name = [array_central_group_vpnc1[0][0] ,array_central_group_vpnc2[0][0],array_central_group_bgw[0][0] ,array_central_group_bgw[0][0]] #Retrieve 1st Array object the group_name
    ...
    print("VPNC's")
    print(VGW_Inventory[['device_name','serial_number','mac_address','group_name','group_id']].head(2))
    print("\nBGW's")
    print(VGW_Inventory[['device_name','serial_number','mac_address','group_name','group_id']].tail(2))
    ...
    VGW = VGW_Inventory.merge(sites_locations, how='inner', left_on=["site_name"], right_on=["site_name"])
    ...
    for (idx, row) in VGW.iterrows():
        ...
        # print(bgw_csv2)
        central.configuration.vgw.download_vgw_userdata(vgw_id,serial_number,device_name) # Download the VGW ISO User-Data.txt files.
        print("\n\n\n")
        print("Sleeping 30 seconds to make sure that the device is added to Central Monitoring Instance - this varies per cid...")
        central.configuration.groups.group_preassignment(serial_number,group_id) #Assign function VPNC and BGW's to the right group.

        
        time.sleep(30)
        central.configuration.sites.sites_assigndevice(site_id, serial_number) #Assign device to monitoring location. This also needs some timing of 10 seconds.
        
        # Push the per-device configuration.
        central.configuration.configuration_push.config_device(filename, device_name, serial_number, mac_address)
...
def sdwan_config():
    ...
    central.configuration.groups.groupconfig_sdwanhub(central_group_bgw, serial_number_vpnc1, serial_number_vpnc2)

    central.configuration.groups.groupconfig_sdwanmesh(central_group_hubmesh)
    central.configuration.groups.groupconfig_sdwanmesh_link(central_group_hubmesh, central_group_vpnc1, central_group_vpnc2)
...
def start_script():
    ...
    for _ in itertools.repeat(None, 4):
        ...
        time.sleep(12)
    # List the VGW's
    print("These VGW's are created: \n{}".format(central.configuration.vgw.get_vgw()))
    ...
    central.configuration.configuration_push.groupconfig_create(central_group_name)
    ...
    match_central_group_bgw = group_list[group_list['group_name'].str.match(central_group_bgw)]
    match_central_group_vpnc1 = group_list[group_list['group_name'].str.match(central_group_vpnc1)]
    match_central_group_vpnc2 = group_list[group_list['group_name'].str.match(central_group_vpnc2)]

    # #Numpy Array of Variable: [SDB-DC4, 50]
    array_central_group_bgw = match_central_group_bgw[["group_name","groupid"]].values
    array_central_group_vpnc1 = match_central_group_vpnc1[["group_name","groupid"]].values
    array_central_group_vpnc2 = match_central_group_vpnc2[["group_name","groupid"]].values
    print(array_central_group_bgw)
    print(array_central_group_vpnc1)
    print(array_central_group_vpnc2)
    ...
    Assignment_VGW(array_central_group_bgw, array_central_group_vpnc1, array_central_group_vpnc2)
...
def delete_script():
    ...
    central.configuration.groups.groupconfig_sdwanmesh_delete(central_group_hubmesh)