...
def get_vgw():
    try:
        ...
        df2 = pd.json_normalize(vvgw_inventory_json['deployments'][0]['vgw_devices'])
        # print(df2)
...
def get_vgw_monitoring():
    try:
        ...
        #     content=response.content))
        vvgw_inventory_json = response.json()
        if vvgw_inventory_json['count'] == 0: # When VGW's have never been online the model status is "MC-VA". If devices have ever been online, they have the model "ArubaVGW".
            try:
                response = requests.get(
                    url= headers.central_instance + "/monitoring/v1/mobility_controllers",
                    headers= headers.set_header_backend(),
    ...
    for serial_number in vgws['serial']:
        try:
            ...
            response = requests.delete(
                url= headers.central_instance + "/monitoring/v1/mobility_controllers/{}".format(serial_number),
                headers= headers.set_header_backend()
            )
            print('Response HTTP Status Code: {status_code}'.format(
                status_code=response.status_code))
...
def delete_vgw_services():
    ...
    for vgw_id in get_vgws['vgw_id']:
        try:
            response = requests.delete(
                url= headers.central_ui_url + "/cgw/vgw_instances/{}".format(vgw_id),
                headers= headers.set_header_frontend()
            )
            print('Response HTTP Status Code: {status_code}'.format(
                status_code=response.status_code))
...
def download_vgw_userdata(vgw_id,serial_number,device_name):
    try:
        response = requests.get(
            url= headers.central_ui_url + "/cgw/vgw_instances/{}/user_data/download_iso".format(vgw_id),
            headers= headers.set_header_frontend(),
        )
        print('Response HTTP Status Code: {status_code}'.format(
            status_code=response.status_code))
